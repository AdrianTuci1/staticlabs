...
function TypewriterText({ text, speed = 46, onComplete }) {
  ...
  return (
    <>
      {visibleText}
      <span className="thunder-chat-demo__caret" aria-hidden="true" />
    </>
  );
}
...
function ResponseBlock({ block, live, delay, onReveal, onTick }) {
  if (block.type === 'csv') {
    return (
      <CrystalBlock delay={delay} onReveal={onReveal}>
        <div className="thunder-chat-demo__csv" aria-label="CSV response table">
          {block.rows.map((row, rowIndex) => (
            <div className="thunder-chat-demo__csv-row" key={row.join('-')}>
              {row.map((cell) => (
                <span key={cell}>
                  <CrystallizedText
                    enabled={live}
  ...
  if (block.type === 'code') {
    return (
      <CrystalBlock delay={delay} onReveal={onReveal}>
        <figure className="thunder-chat-demo__code">
...
function AssistantMessage({ response, live, onReveal, onTick }) {
  return (
    <article className="thunder-chat-demo__message thunder-chat-demo__message--assistant">
      <span className="thunder-chat-demo__role">Thunder</span>
...
export function ThunderChatDemo() {
  ...
  return (
    <section className="thunder-chat-demo" aria-label="Thunder chat visualizer">
      <div className="thunder-chat-demo__messages" ref={messagesRef}>
        <div className="thunder-chat-demo__messages-inner">
          {DEMO_SCRIPT.slice(0, activeIndex).map((scene) => (
            <div className="thunder-chat-demo__exchange" key={scene.prompt}>
              <article className="thunder-chat-demo__message thunder-chat-demo__message--user">
                <p>{scene.prompt}</p>
              </article>
          ...
          {phase === 'responding' && (
            <div className="thunder-chat-demo__exchange">
              <article className="thunder-chat-demo__message thunder-chat-demo__message--user">
      ...
      <div
        className={`thunder-chat-demo__composer-wrap ${activeIndex === 0 && (phase === 'typing' || phase === 'sending') ? 'is-centered' : ''}`}
      >
        {activeIndex === 0 && (phase === 'typing' || phase === 'sending') && (
          <div className="thunder-chat-demo__brand" aria-hidden="true">
            <LabMarkLogo className="thunder-chat-demo__brand-glyph" />
            <span>static labs</span>
          </div>
        )}

        <div className={`thunder-chat-demo__composer ${phase === 'sending' ? 'is-sending' : ''}`}>
          <div className="thunder-chat-demo__prompt">
            <span>
              {phase === 'typing' ? (
          ...
          <div className="thunder-chat-demo__meta" aria-hidden="true">
            <span className="thunder-chat-demo__icon-button">
              <Earth size={14} strokeWidth={2} />
            </span>
            <span className="thunder-chat-demo__reasoning">
              <BrainCircuit size={14} strokeWidth={2} />
              {THUNDER_REASONING}
            </span>
            <span className="thunder-chat-demo__model">{THUNDER_MODEL}</span>
            <button className="thunder-chat-demo__send" type="button" tabIndex={-1} aria-label="Send demo prompt">
